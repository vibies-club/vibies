...
create table if not exists vibies_private.project_milestones (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null
    references vibies_private.personal_projects (id) on delete cascade,
  title text not null
    check (vibies_private._valid_project_text(title, 80, false)),
  completed boolean not null default false,
  blocked_note text
    check (
      blocked_note is null
      or vibies_private._valid_project_text(blocked_note, 500, true)
    ),
  position integer not null check (position between 1 and 20),
  created_at timestamptz not null default clock_timestamp(),
  updated_at timestamptz not null default clock_timestamp(),
  check (not completed or blocked_note is null),
  constraint project_milestones_position_unique
    unique (project_id, position) deferrable initially deferred
);

create or replace function vibies_private._project_roadmap(p_project_id uuid)
returns jsonb
language sql
stable
set search_path = pg_catalog, vibies_private
as $$
    ...
    select coalesce(
             jsonb_agg(
               jsonb_build_object(
                 ...
                 'blockedNote', m.blocked_note
               ) order by m.position
             ),
             '[]'::jsonb
           ) as milestones,
           count(*)::integer as total_count,
           count(*) filter (where m.completed)::integer as completed_count
      from vibies_private.project_milestones m
  ...
  select jsonb_build_object(
    ...
    'percentage', case
      ...
      else round(completed_count * 100.0 / total_count)::integer
    end
  )
    from roadmap;
$$;
...
begin
  ...
  if p_action = 'approve' then
    ...
    -- ponytail: one singleton row serializes this nine-person community; use a
    -- dedicated capacity lock only if membership throughput ever matters.
    if (select count(*) from vibies_private.accounts where status = 'approved') >= 8 then
...
create or replace function vibies_private.add_project_milestone(
  p_session_hash text,
  p_project_id uuid,
...
begin
  if not vibies_private._valid_hash(p_session_hash)
     ...
     or (
       p_blocked_note is not null
       and not coalesce(vibies_private._valid_project_text(p_blocked_note, 500, true), false)
     ) then
    return jsonb_build_object('kind', 'invalid');
  end if;
  ...
  if milestone_count >= 20 then
    return jsonb_build_object('kind', 'full');
  end if;

  insert into vibies_private.project_milestones (
    project_id, title, blocked_note, position
  ) values (
    p_project_id, p_title, p_blocked_note, milestone_count + 1
  );
  update vibies_private.personal_projects
     set updated_at = clock_timestamp(), version = version + 1
   where id = p_project_id;
  return jsonb_build_object('kind', 'added');
end;
$$;

create or replace function vibies_private.edit_project_milestone(
  p_session_hash text,
  p_project_id uuid,
  p_milestone_id uuid,
  p_expected_version bigint,
...
begin
  if not vibies_private._valid_hash(p_session_hash)
     or p_project_id is null or p_milestone_id is null
     or p_expected_version is null or p_expected_version < 1
     or not coalesce(vibies_private._valid_project_text(p_title, 80, false), false)
     or (
       p_blocked_note is not null
  ...
  select title, blocked_note, completed
    into milestone_title, milestone_note, milestone_completed
    from vibies_private.project_milestones
   where id = p_milestone_id and project_id = p_project_id
   for update;
  if not found or (milestone_completed and p_blocked_note is not null) then
    return jsonb_build_object('kind', 'invalid');
  end if;
  if milestone_title = p_title and milestone_note is not distinct from p_blocked_note then
    return jsonb_build_object('kind', 'unchanged');
  end if;

  update vibies_private.project_milestones
     set title = p_title,
         blocked_note = p_blocked_note,
...
create or replace function vibies_private.set_project_milestone_completion(
  p_session_hash text,
  p_project_id uuid,
  p_milestone_id uuid,
  p_expected_version bigint,
...
begin
  ...
  select completed into milestone_completed
    from vibies_private.project_milestones
   where id = p_milestone_id and project_id = p_project_id
   for update;
  if not found then
  ...
  if milestone_completed = p_completed then
    return jsonb_build_object('kind', 'unchanged');
  end if;

  update vibies_private.project_milestones
     set completed = p_completed,
         blocked_note = case when p_completed then null else blocked_note end,
         updated_at = clock_timestamp()
   where id = p_milestone_id;
  update vibies_private.personal_projects
     set updated_at = clock_timestamp(), version = version + 1
   where id = p_project_id;
  return jsonb_build_object(
    'kind', case when p_completed then 'completed' else 'reopened' end
  );
end;
$$;

create or replace function vibies_private.move_project_milestone(
  p_session_hash text,
  p_project_id uuid,
  p_milestone_id uuid,
  p_expected_version bigint,
...
begin
  ...
  if not exists (
    select 1 from vibies_private.project_milestones
     where project_id = p_project_id and position = target_position
  ) then
    return jsonb_build_object('kind', 'unchanged');
  end if;
  ...
  update vibies_private.project_milestones
     set position = case
       when id = p_milestone_id then target_position
       else current_position
     end,
         updated_at = clock_timestamp()
   where project_id = p_project_id
     and position in (current_position, target_position);
  update vibies_private.personal_projects
     set updated_at = clock_timestamp(), version = version + 1
   where id = p_project_id;
  return jsonb_build_object('kind', 'moved');
end;
$$;

create or replace function vibies_private.delete_project_milestone(
  p_session_hash text,
  p_project_id uuid,
  p_milestone_id uuid,
  p_expected_version bigint
...
begin
  ...
  delete from vibies_private.project_milestones where id = p_milestone_id;
  update vibies_private.project_milestones
     set position = position - 1,
         updated_at = clock_timestamp()
   where project_id = p_project_id and position > removed_position;
  update vibies_private.personal_projects
     set updated_at = clock_timestamp(), version = version + 1
   where id = p_project_id;
  ...
  select coalesce(jsonb_agg(item order by created_at, id), '[]'::jsonb)
    ...
    from (
      select p.created_at, p.id, jsonb_build_object(
        ...
        'progressPercentage', vibies_private._project_roadmap(p.id) -> 'percentage'
      ) as item
        from vibies_private.personal_projects p
...
begin
  ...
  details := jsonb_build_object(
    ...
    'roadmap', vibies_private._project_roadmap(project_row.id)
  );
...
grant execute on function vibies_private.add_project_milestone(text, uuid, bigint, text, text) to vibies_runtime;
grant execute on function vibies_private.edit_project_milestone(text, uuid, uuid, bigint, text, text) to vibies_runtime;
grant execute on function vibies_private.set_project_milestone_completion(text, uuid, uuid, bigint, boolean) to vibies_runtime;
grant execute on function vibies_private.move_project_milestone(text, uuid, uuid, bigint, text) to vibies_runtime;
grant execute on function vibies_private.delete_project_milestone(text, uuid, uuid, bigint) to vibies_runtime;
